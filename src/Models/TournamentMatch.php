...
class TournamentMatch extends BaseModel
{
    ...
    {
        $team1Id = $this->team1_id;
        $team2Id = $this->team2_id;
    ...
    private function advanceWinner(int $winnerId): void
    {
        ...
        error_log("Found " . count($nextMatches) . " next matches");
        
        if (empty($nextMatches)) {
            error_log("No next match found!");
            return;
        }
        
        $nextMatch = $nextMatches[0];
        error_log("Next match ID: " . $nextMatch['id']);
        ...
        error_log("Setting winner in field: $teamField (odd match: " . ($isOddMatch ? 'yes' : 'no') . ")");
        
        // Aggiorna la partita successiva
        $result = $db->update('matches', $nextMatch['id'], [
            $teamField => $winnerId,
        ]);
        
        error_log("Update result: " . ($result ? 'SUCCESS' : 'FAILED'));
        error_log("=== advanceWinner END ===");
    }
